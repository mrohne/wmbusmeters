...
bool logfile_enabled_ = true;
bool logging_silenced_ = true;
bool verbose_enabled_ = true;
bool debug_enabled_ = true;
bool trace_enabled_ = true;
AddLogTimestamps log_timestamps_ {};
bool stderr_enabled_ = true;
bool log_telegrams_enabled_ = true;
bool internal_testing_enabled_ = true;
...
void info(const char* fmt, ...) {
    if (!logging_silenced_) {
        va_list args;
        va_start(args, fmt);
        esp_log_writev(ESP_LOG_INFO, "WMBUS", fmt, args);
        va_end(args);
    }
}
...
void warning(const char* fmt, ...) {
    if (!logging_silenced_) {
        va_list args;
        va_start(args, fmt);
        esp_log_writev(ESP_LOG_WARN, "WMBUS", fmt, args);
        va_end(args);
    }
}
...
void error(const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    esp_log_writev(ESP_LOG_ERROR, "WMBUS", fmt, args);
    va_end(args);
}
...
{
    warning("Could not list files %s\n", dir.c_str());
    return false;
}
